...
namespace YoutubeData {
	...
	void GenNumAccessesByVideos() {
		Cons::MT _("Gen twitter user IDs ...");

		set<long> uids;
		for (const Op* op0: _entries) {
			const OpYoutube* op = static_cast<const OpYoutube*>(op0);
			uids.insert(op->uid);
		}
		...
		for (auto i: uids) {
			if (j % 1000 == 0) {
				if (j != 0)
					ofs << ";\n";
				ofs << "INSERT IGNORE INTO twitter4.users (id, gen, status) VALUES (" << i << ", -1, 'U')";
			} else {
				ofs << ", (" << i << ", -1, 'U')";
			}
			j ++;
		...
		+--------------+-------------+------+-----+---------+-------+
			...
			| gen          | int(11)     | NO   | MUL | NULL    |       |
			| added_at     | timestamp   | YES  |     | NULL    |       |
			| crawled_at   | timestamp   | YES  | MUL | NULL    |       |
			| status       | varchar(2)  | NO   | MUL | NULL    |       |
			| check_out_at | timestamp   | YES  | MUL | NULL    |       |
			| check_out_ip | varchar(20) | YES  |     | NULL    |       |
			+--------------+-------------+------+-----+---------+-------+
		*/
