...
namespace Conf {
	...
	void _ParseArgs(int argc, char* argv[]) {
		...
		od.add_options()
			...
			("dist_sum_sq_threshold",
			 po::value<double>()->default_value(Conf::Get("dist_sum_sq_threshold").as<double>()))
		...
		__EditYaml<double>("dist_sum_sq_threshold", vm);
	}
	...
	void Init(int argc, char* argv[]) {
		...
		double dist_sum_sq_threshold = Get("dist_sum_sq_threshold").as<double>();
		...
		fn_plot_data = str(boost::format("%s/%s-clustered-with-dist-sq-%.3f")
				% dn_plot_data
